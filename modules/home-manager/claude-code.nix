...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    home.packages =
      (lib.optional (!useStatuslineBinary) pkgs.bc)
    ...
    home.file = {
      ...
      ".claude/custom-plugins/lsp-servers/.lsp.json" =
        lib.mkIf (cfg.lsp.enable && cfg.lsp.servers != { })
          {
    ...
    programs.claude-code.settings = lib.mkIf cfg.statusLine.enable {
      statusLine = {
        type = "command";
        command =
          if useStatuslineBinary then
            "${lib.getExe cfg.statusLine.package}"
          else
            "${config.home.homeDirectory}/.claude/statusline-command.sh";
      };
    };
